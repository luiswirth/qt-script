...
kinetic energy and is the Born-Oppenheimer approximation;
...
$ <single-electron>
with $psi(avec(r))$ #unit($m^(-3 slash 2)$) depending on one position only.
Everything the other electrons do to the one under consideration is cast into
the single #term("effective potential") $V_"eff" (avec(r))$ #unit($J$).
...
named $avec(k)$ #unit($m^(-1)$) and called the #term("wave vector"),
$
  psi_avec(k) (avec(r) + n avec(a)) = e^(i avec(k) dot n avec(a)) psi_avec(k) (avec(r)).
$ <bloch-phase>
Bloch's theorem is the statement that a function with this property is a plane
wave times something with the periodicity of the lattice,
$
...
one for each $avec(k)$, and the function $E(avec(k))$ that results is the
#term("bandstructure").
...
The band diagram, which appears below, is energy against position, and it
belongs to a structure assembled from several materials.
...
$T$ #unit($K$) the temperature,
and $E_F$ #unit($J$) the #term("Fermi level"), the energy at which the occupation
is exactly one half.
...
Its units are those of a mass, and it is quoted as a multiple of $m_0$.
...
$ <ema>
which is the form used for the rest of the course.
...
into $m^*$.
$V_"ext" (avec(r))$ #unit($J$) is everything the lattice does not supply:
the electrostatic potential generated by the charges in the device, obtained
from Poisson's equation, and the band offsets where materials of different gap
meet.
...
along its own axis with a longitudinal mass $m_L = 0.91 m_0$ along that axis and
a transverse mass $m_T = 0.19 m_0$ along the two perpendicular to it.
...
The periodic boundary condition is a device for counting states in a finite
volume and not a physical statement;
...
The band diagram is then constructed by walking along $x$ and recording, at each
position, the band edge of whichever material is there.
...
$phi prop e^(-kappa x)$ with decay constant
$
...
zero.
This penetration is the wave function penetration of slide 38 of the previous
...
$ <hopping>
in which $t$ #unit($J$) is the only parameter, an energy set by the grid
